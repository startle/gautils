...
def read_dicts(s: str, kv_split='=', split='&', parse_unquote=url_parse_unquote):
    d = {}
    if parse_unquote is None:
        def parse_unquote(x): return x
        ...
        if os.path.isfile(path):
            ...
            for cpath in os.listdir(path):
                for x in recursion_list_file(os.path.join(path, cpath)):
                    yield x
    if _dir is None :
        raise Exception('_dir cannot be None.')
    if not os.path.exists(_dir) or not os.path.isdir(_dir):
        raise Exception('_dir is not a dir. [%s]' % _dir)

    if not recursion:
        return os.listdir(_dir)
    else:
        return list(recursion_list_file(_dir))
