    ...
    def query(self) -> pd.DataFrame:
        ...
        try :
            if not self.__where.empty():
                ...
                return self.__db.query(self.__table, sql, *(self.__where.params()))
            else :return self.__db.query(self.__table,sql)
        except Exception as ex:
            log.error('error sql: %s' % sql, exc_info=ex)
            raise ex
class Db:
    def __init__(self, h, p, u, pwd,db, **kws):
        self.__host = h
        self.__port = p
        self.__pwd = pwd
        self.__user = u
        try:
            ...
            self.__conn = mysql.connector.connect(host=h, port=p, user=u, passwd=pwd, database=db, charset='utf8')
        except Exception as e:
            log.error('---------offline mode running... read from cache.', exc_info=e)
    def close(self):
        self.__conn.close()
    def s_query(self, table, cols=None) -> Query:
        return Query(self, table, cols)
    def execute(self, sql, is_close_conn=True, *params):
        cursor = self.__conn.cursor()
        cnt = cursor.execute(sql, params)
        self.__conn.commit()
        cursor.close()
        if is_close_conn : self.conn_close()
        return cnt
    def __dbtype2dtype(self, dbtype:str):
        index = dbtype.find('(')
    ...
    def query(self, table:str, sql:str, *params, is_close_conn=True):
        def read_row(row):
            def trans(x):
        ...
        def query_from_db():
            desc_df = self.describe(table, is_close_conn=False)
            cursor = self.__conn.cursor()
            cursor.execute(sql, params)
            rows = cursor.fetchall()
            cols = [x[0] for x in cursor.description]
            df = pd.DataFrame(data = [read_row(row) for row in rows], columns = cols)
            desc_df = desc_df.set_index(['Field'])
            dbtypes = {c : desc_df.loc[c, 'Type'] for c in cols}
            dtypes = {c : self.__dbtype2dtype(dbtypes[c]) for c in cols}
            df = df.astype(dtypes)
            cursor.close()
            if is_close_conn : self.conn_close()
            return df
        return query_from_db()
    def describe(self, table, is_close_conn = True):
        cursor = self.__conn.cursor()
        ...
        if is_close_conn: self.conn_close()
        return df
    def keys_cols(self, table, is_close_conn = True):
        df:pd.DataFrame = self.describe(table, is_close_conn = is_close_conn)
    ...
    def update(self, table, df:pd.DataFrame, batch=10000, is_close_conn=True):
        df = df.reset_index()
        cnt = 0
        for idf in Db._section_batch(df, batch):
    ...
    def _update(self, table, df:pd.DataFrame, is_close_conn=True):
        ...
        idx_cols, upd_cols= self.keys_cols(table, is_close_conn=False)
        ...
        try:
            ...
            if is_close_conn : self.conn_close()
            return cnt
    def delete_all(self, table, is_close_conn = True):
        sql = 'DELETE FROM {0}'.format(table)
        return self.execute(sql, is_close_conn = is_close_conn)
    def conn_close(self):
        self.__conn.close()
def connect(h, p, u, pwd, db, is_use_file_cache = False, cache_dir=None, charset='utf8'):
    db = Db(h, p, u, pwd, db, charset=charset)
    ...
    def file_cache_query(table:str, sql:str, *params):
        ...
        if is_use_file_cache and os.path.isfile(cache_path):
            ...
            df = Db.query(db, table, sql, *params)
